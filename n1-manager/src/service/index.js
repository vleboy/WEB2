...
const post = (urls, datas, type) => {
    let TOKEN='';
    let headers={};
    if(window.localStorage.getItem('managerToken')) {
        TOKEN = window.localStorage.getItem('managerToken')
        headers={
            "Content-Type": "application/json; charset=utf-8",
            "Authorization":TOKEN
        }
...
//登录
export async function logIn(role,username,password,suffix,challenge,vid,cb,err){
    return http(post('/users/auth',{role,username,password,suffix,challenge,vid,cb,err}))
}
//查询平台用户账单统计
