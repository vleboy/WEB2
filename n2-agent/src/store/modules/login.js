    ...
    actions:{
        ...
        userlogin({commit},{role,username,password,challenge,vid,cb,err}){
            logIn(role,username,password,challenge,vid).then(res=>{
                // console.log(res);
                if(res.code==0){
                    if(localStorage.getItem('n2agentToken')) {
                        localStorage.removeItem('n2agentToken');
                        localStorage.setItem('n2agentToken', res.payload.token)
                    }else{
                        localStorage.setItem('n2agentToken', res.payload.token);
                    }
                    setTimeout(()=>localStorage.removeItem('n2agentToken'),259200000);
